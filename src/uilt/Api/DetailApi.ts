...
export const DetailHomeAxios = async(detail:string , id:string , type:string) => {
    if(type === 'GetTheSongsDirectly') {
        ...
        const fetchSong = fetchDetail.data.dailySongs
        ...
        useStore().Detail.getMusicSongsDetailList({DetailTitle: fetchTitle, DetailSong: fetchDetail.songs})
    }
...
export const DetailSearchAxios = async (detail: string) => {
    ...
    useStore().Detail.getMusicSongsDetailList({DetailTitle:{} , DetailSong:fetchDetail.result.songs})
...
export const DetailThatLove = async () => {
    ...
    const fetchTitle:MusicDetailData = {img:fetchSongs?.songs[0].al.picUrl,title:'我喜欢的音乐',Label:['喜欢','个人'],BriefIntroduction:'每当我打开我的音乐库，都像是进入了一个神秘的音乐王国，我可以尽情享受其中的声音和旋律',MusicLength:'播放全部' + fetchSongs.songs.length}
